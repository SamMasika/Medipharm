			...
			<v-card prepend-icon="mdi-plus" title="Add Category">
					<v-form>
							<v-card-text>
									<v-row dense>
											<v-col cols="12" sm="12" md="12">
													<v-text-field label="Name*" v-model="category.name" required variant="outlined" density="compact"></v-text-field>
											</v-col>
									</v-row>
									<v-row dense>
											<v-col cols="12" sm="12" md="12">
													<v-text-field label="Description*" v-model="category.description" required variant="outlined" density="compact"></v-text-field>
											</v-col>
									</v-row>
							</v-card-text>
							<v-divider></v-divider>
							<v-card-actions>
									...
									<v-btn type="submit" text="Save" class="text-none button-color" variant="flat" @click="addCategory" rounded="xl"></v-btn>
							</v-card-actions>
					</v-form>
			</v-card>
	</v-dialog>
<v-container fluid v-if="itemsLength > 0">
    ...
    <v-card flat>
        ...
        <DataTable :api-url="'/category-list'" :headers="headers">
    ...
    <v-dialog v-model="categoryEditDialog" max-width="600">
        <v-card prepend-icon="mdi-plus" title="Update Category">
            <v-form>
                <v-card-text>
                    ...
                    <v-row dense>
                        <v-col cols="12" sm="12" md="12">
                            <v-text-field label="Description*" v-model="categoryEdit.description" required variant="outlined" density="compact"></v-text-field>
                        </v-col>
                    </v-row>
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn text="Close" class="text-none" variant="tonal" @click="categoryEditDialog = false" rounded="xl"></v-btn>
                    <v-btn type="submit" text="Save" class="text-none button-color" variant="flat" @click="updateCategory" rounded="xl"></v-btn>
                </v-card-actions>
            </v-form>
        </v-card>
    </v-dialog>
        ...
        <v-card class="rounded-lg elevation-16" style="background-color: #f9f9f9;">
            ...
            <v-card-text class="text-center py-1" style="font-family: 'Roboto', sans-serif; font-size: 16px; line-height: 1.6;">
                ...
                <div class="font-weight-medium text-body-1 text-center mb-4">
                    Are you sure you want to delete <b>"{{ itemToDelete.categoryName }} </b>?
                </div>
...
<v-container class="relative-container" fluid v-else>
    <div class="centered-message">
        <v-card class="pa-6 text-center" elevation="0" max-width="500">
            ...
            <h2 class="mt-4 mb-2 text-grey-darken-2">No Categories Data Found</h2>
            <p class="text-grey">
                There are no categories records found at the moment.
            </p>
            <v-btn class="text-none font-weight-regular button-color my-5" prepend-icon="mdi-plus" text="Add Category" variant="flat" @click="dialog = true" rounded="xl"></v-btn>
...
export default {
    ...
    methods: {
fetchItems() {
            axios.get('/category-list') // Replace with your actual API URL
                .then(response => {
