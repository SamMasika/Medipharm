...
<v-dialog v-model="dialog" max-width="900">
    <v-card prepend-icon="mdi-plus" title="Add Staff">
        <v-form>
            <v-card-text>
                <v-row>
                    <v-col cols="12" md="6">
                        <v-text-field v-model="staff.name" label="Full Name*" prepend-inner-icon="mdi-account" variant="outlined" required />
                    </v-col>
                    <v-col cols="12" md="6">
                        <v-text-field v-model="staff.username" label="Username*" prepend-inner-icon="mdi-account-circle" variant="outlined" required />
                    </v-col>
                </v-row>
                <v-row>
                    <v-col cols="12" md="6">
                        <v-text-field v-model="staff.email" label="Email*" prepend-inner-icon="mdi-email" type="email" variant="outlined" required />
                    </v-col>
                    <v-col cols="12" md="6">
                        <v-text-field v-model="staff.phone" label="Phone Number*" prepend-inner-icon="mdi-phone" variant="outlined" required />
                    </v-col>
                </v-row>
                <v-row>

                    <v-col cols="12" md="6">
                        <v-text-field v-model="staff.password" :type="showPassword ? 'text' : 'password'" label="Password*" prepend-inner-icon="mdi-lock" :append-inner-icon="showPassword ? 'mdi-eye-off' : 'mdi-eye'" @click:append-inner="showPassword = !showPassword" variant="outlined" :error-messages="passwordError" @input="validatePassword" required />
                    </v-col>
                    <v-col cols="12" md="6">
                        <v-text-field v-model="staff.password_confirmation" :type="showPassword ? 'text' : 'password'" label="Confirm Password*" prepend-inner-icon="mdi-lock-check" :append-inner-icon="showPassword ? 'mdi-eye-off' : 'mdi-eye'" @click:append-inner="showPassword = !showPassword" variant="outlined" :error-messages="confirmPasswordError" @input="validateConfirmPassword" required />
                    </v-col>
                </v-row>
            </v-card-text>
            <v-divider></v-divider>
            <v-card-actions>
                ...
                <v-btn type="submit" text="Save" class="text-none button-color" variant="flat" @click="addStaff" rounded="xl"></v-btn>
            </v-card-actions>
        </v-form>
    </v-card>
</v-dialog>
<v-container fluid v-if="itemsLength > 0">
    ...
    <v-row justify="end">
        <v-col cols="12" md="auto" class="d-flex justify-end">
            <v-btn class="text-none font-weight-regular button-color my-5" prepend-icon="mdi-plus" text="Add Staff" variant="flat" @click="dialog=true" rounded="xl"></v-btn>
        </v-col>
    </v-row>
    <v-card flat>
        <v-toolbar>
            <v-icon icon="mdi-account-multiple-outline" class="mx-5 custom-icon" size="40"></v-icon> &nbsp;
            Staff
            <v-spacer></v-spacer>
        </v-toolbar>
        <DataTable :api-url="'/users/company'" :headers="headers">
    ...
    <v-dialog v-model="staffEditDialog" max-width="900">
        <v-card prepend-icon="mdi-plus" title="Update Supplier">
            <v-form>
                <v-card-text>
                    <v-row dense>
                        <v-col cols="12" sm="6" md="6">
                            <v-text-field placeholder="Name*" v-model="staffEdit.fullName" required variant="outlined"></v-text-field>
                        </v-col>
                        <v-col cols="12" sm="6" md="6">
                            <v-text-field placeholder="Contact Person*" v-model="staffEdit.contact_person" required variant="outlined"></v-text-field>
                        </v-col>
                    </v-row>
                    <v-row dense>
                        <v-col cols="12" sm="6" md="6">
                            <v-text-field placeholder="Phone*" v-model="staffEdit.mobileNo" required variant="outlined"></v-text-field>
                        </v-col>
                        <v-col cols="12" sm="6" md="6">
                            <v-text-field placeholder="Email*" type="email" v-model="staffEdit.email" required variant="outlined"></v-text-field>
                        </v-col>
                    </v-row>
                    <v-row dense>
                        <v-col cols="12" sm="12" md="12">
                            <v-text-field placeholder="Location*" v-model="staffEdit.location" required variant="outlined"></v-text-field>
                        </v-col>
                    </v-row>
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn text="Close" class="text-none" variant="tonal" @click="staffEditDialog = false" rounded="xl"></v-btn>
...
<v-container class="relative-container" fluid v-else>
    <div class="centered-message">
        <v-card class="pa-6 text-center" elevation="0" max-width="500">
            ...
            <h2 class="mt-4 mb-2 text-grey-darken-2">No Staff Data Found</h2>
            <p class="text-grey">
...
export default {
    ...
    methods: {
        fetchItems() {
            axios.get('/users/company') // Replace with your actual API URL
                .then(response => {
        ...
        updateSupplier() {
            ...
            axios.put(`/supplier-update/${id}`, {
                ...
                .then(response => {
                    this.showAlert(response.data.message, 'success');
                    this.staffEditDialog = false; // Close the dialog after success
                    setTimeout(() => {
                        window.location.reload(); // Reload the window after success
                    }, 500); // Delay the reload slightly to allow the success message to be shown
                })
        ...
        deleteItem() {
            axios.delete(`/supplier-delete/${this.itemToDelete.id}`)
                .then(response => {
                    ...
                    const index = this.staffs.indexOf(this.itemToDelete);
                    if (index > -1) {
